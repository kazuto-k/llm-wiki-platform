...
Git リポジトリ（llm-wiki schema の Markdown 群）
  ├── entities/*.md       # 人物・組織・プロジェクト・技術 etc
  ├── concepts/*.md       # 概念・ベストプラクティス・ガイド
  ├── comparisons/*.md    # 比較表・選定資料
  └── meta/schema.yaml    # llm-wiki schema 定義
      ↓ Git 双方向同期（Deploy Key）
Wiki.js（人間向け UI + ACL + 全文検索）
      ↕ GraphQL API
llm-wiki エージェント（CI パイプラインとして実行）
```

raw は「場所」ではなく frontmatter の `status: raw` という「状態」。
connector が最初に正しいパスに配置し、curator が同一ファイルを `status: curated` に変換する。
